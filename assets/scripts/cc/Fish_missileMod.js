cc.Class({
    ...
    onCollisionEnter: function(other) {
        ...
        "plane" != nodeName || this.runAction || this.onRunAction(), "BigBoss" == nodeName ? this.onBlast() : "puffer" == nodeName || "point5" == nodeName || "stone01" == nodeName || "point4" == nodeName || "point3" == nodeName || "point2" == nodeName || "point1" == nodeName || "pufferMod" == nodeName ? this.onBlast() : "Missile" == nodeName && this.node.x > other.node.x && this.onBlast();
    },
    onBlast: function() {
    ...
    update: function(dt) {
        if (window.gameCrtl && !window.gameCrtl.isPause) {
            ...
            this.node.y += 400 * dt * ration, (2e3 <= this.node.y || -2e3 >= this.node.y) && this.node.parent.destroy();
        }
    }
});